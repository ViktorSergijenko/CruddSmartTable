...
namespace TESTAPI.Controllers
{
    ...
    public class HouseController : Controller
    {
        ...
        public IEnumerable<House> GetAll()
        {
           ...
           Init();
            var houses = _context.Houses
                ...
                .ToList();
                houses.ForEach(house => house.flatamount = house.flats.Count);
            return houses;
        }
                ...
                for (int houseInc = 0; houseInc < 5; houseInc++)
                {
                    ...
                    for (int flatInc = 0; flatInc < 5; flatInc++)
                    {
                        var newFlat = new Flat { number = flatInc, floor = r.Next(1, 5), livingspace = r.Next(100, 300) / 11, totalarea = r.Next(100, 300) / 9};
                          ...
                          {
                            newFlat.residents.Add(new Resident { firstname = $"Bruce-{resInc}",
                              lastname = "Wayne"
                              , postcode = $"LV3{0}{r.Next(0,9)}{r.Next(0, 9)}",
                              phone = $"BatSignal or - 2{r.Next(0, 9)}{r.Next(0, 9)}{r.Next(0, 9)}{r.Next(0, 9)}{r.Next(0, 9)}{r.Next(0, 9)}{r.Next(0, 9)}",
                              email = $"superman{r.Next(0, 20)}@sucks{r.Next(0, 9)}.com" });
        ...
        public IActionResult Create([FromBody] House item)
        {
            ...
            item.flatamount = item.flats.Count;
        ...
        public IActionResult Update(long id, [FromBody] House item)
        {
            ...
            House.flatamount = House.flats.Count;
